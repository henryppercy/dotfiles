...
function M.find_current_scope()
  ...
  if not node_at_cursor then return M.scopes.Posting end
  local node_type = node_at_cursor:type()

  if node_type == "account" then
    return M.scopes.Account
  elseif node_type == "commodity" then
    return M.scopes.Commodity
  elseif node_type == "posting" then
    return M.scopes.Posting
  else
    return M.scopes.Posting
  end
end
...
function M.get_commodities_from_source(node, source, filename, ctx)
  -- The ledger treesitter grammar's commodity_directive query captures inner
  -- format nodes rather than the currency symbol, so use text scanning instead.
  ctx.commodities[filename] = {}
  local seen = {}
  for line in source:gmatch("[^\n]+") do
    local sym = line:match("^commodity%s+([^%d%s,]+)")
             or line:match("^D%s+([^%d%s,]+)")
    if sym and not seen[sym] then
      seen[sym] = true
      table.insert(ctx.commodities[filename], sym)
    end
  end
end
